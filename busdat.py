...
def cost(a,b):
    ...
    cursor = SQLC.cursor()
    exn = 'SELECT B_Fare * %s from bus where B_No =%s'
...
def prnt():
    import docx
    doc = docx.Document()
    doc.add_heading('Bus Bookings', 0)
    ...
    DT4 = 'Bus Name : ',a3
    DT5 = 'Bus Boarding : ',a4
    DT6 = 'Bus Destination : ',a5
    DT7 = 'Bus Departure Time : ',a6
...
def data(a):
    ...
    SQLC = mysql.connector.connect(host = lchst,user = uname,passwd = pword,database = 'booking')
    cursor = SQLC.cursor()
    exn = 'SELECT * from bus where B_No =%s'
