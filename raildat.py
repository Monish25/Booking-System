...
def cost(a,b):
    ...
    cursor = SQLC.cursor()
    exn = 'SELECT T_Fare * %s from rail where T_No =%s'
...
def prnt():
    import docx
    doc = docx.Document()
    doc.add_heading('Train Bookings', 0)
    ...
    DT3 = 'Fare :',a2,'.INR'
    DT4 = 'Train Name : ',a3
    DT5 = 'Train Boarding : ',a4
    DT6 = 'Train Destination : ',a5
    DT7 = 'Train Departure Time : ',a6
...
def data(a):
    ...
    SQLC = mysql.connector.connect(host = lchst,user = uname,passwd = pword,database = 'booking')
    cursor = SQLC.cursor()
    exn = 'SELECT * from rail where T_No =%s'
...
def chk2(a):
        ...
        sql_select_query = """select T_No from rail where T_No = %s"""
...
E4 = tk.Entry(D1) 
L5 = tk.Label(D1,text='Train Number')
E5 = tk.Entry(D1)
